...
class MemeStore {
  ...
  vote(meme, type, token) {
    this.api.vote(meme._id, type, token).then(res => {
      ...
      if (res.success) {
        this.memesLoaded[index] = res.meme; // change to be equal to res if successful
      }else {
        console.log('Sorry rakk ikke legge inn noe bedre enn alert for å gi deg denne beskjeden :(');
        alert('Du kan ikke stemme det samme to ganger! \n Sorry rakk ikke legge inn noe bedre enn alert for å gi deg denne beskjeden :(');
      }
